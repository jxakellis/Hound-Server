...
import { formatDate, formatNumber, formatUnknownString } from '../../main/format/formatObject';
...
async function createUser(req: express.Request, res: express.Response) {
  try {
    const { databaseConnection } = req.extendedProperties;
    // TODO NOW validatedUserIdentifier won't be not nil because its set inside the validateUserId middleware (which isn't invoked here)
    const { validatedUserIdentifier } = req.extendedProperties.validatedVariables;
  ...
  async function updateUser(req: express.Request, res: express.Response) {
    try {
      ...
      if (validatedUserId === undefined) {
        throw new HoundError('validatedUserId missing', 'createUser', ERROR_CODES.VALUE.INVALID);
      }
      
    
      const result = await updateUserForUserId(
        databaseConnection,
        validatedUserId
        { 
