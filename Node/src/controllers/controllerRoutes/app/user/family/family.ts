...
async function getFamily(req: express.Request, res: express.Response): Promise<void> {
  try {
    ...
    const { authFamilyActiveSubscription } = req.houndProperties.authenticated;
    if (authFamilyActiveSubscription === undefined || authFamilyActiveSubscription === null) {
      throw new HoundError('authFamilyActiveSubscription missing', getFamily, ERROR_CODES.VALUE.MISSING);
    }

    const result = await getAllFamilyInformationForFamilyId(databaseConnection, authFamilyId, authFamilyActiveSubscription);
...
async function createFamily(req: express.Request, res: express.Response): Promise<void> {
  try {
    ...
    const result = await createFamilyForUserId(databaseConnection, authUserId);
...
async function updateFamily(req: express.Request, res: express.Response): Promise<void> {
  try {
    ...
    await updateFamilyForUserIdFamilyId(databaseConnection, authUserId, authFamilyId, familyCode, familyIsLocked);
...
async function deleteFamily(req: express.Request, res: express.Response): Promise<void> {
  try {
    ...
    const { authFamilyActiveSubscription } = req.houndProperties.authenticated;
    if (authFamilyActiveSubscription === undefined || authFamilyActiveSubscription === null) {
      throw new HoundError('authFamilyActiveSubscription missing', deleteFamily, ERROR_CODES.VALUE.MISSING);
    }

    if (familyKickUserId !== undefined && familyKickUserId !== null) {
      await kickFamilyMemberForUserIdFamilyId(databaseConnection, authUserId, authFamilyId, familyKickUserId);
    }
    else {
      await deleteFamilyLeaveFamilyForUserIdFamilyId(databaseConnection, authUserId, authFamilyId, authFamilyActiveSubscription);
    }
