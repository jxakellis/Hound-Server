const { serverLogger } from './loggers';
...
const { formatString } from '../format/formatObject';
...
async function logServerError(forFunction, forError) {
  ...
  // Attempt to get the .stack. If the stack is null, then we just simply get the error
